...
from dice.alg import mdn, mdnsr, mdnlr, binomial
...
def parse_and_eval_dice(expr: str):
    ...
    def summarizer(dice_expr, dices, val):
        if isinstance(dice_expr, DiceExpr):
            ...
            return f'{dice_expr}: [' + ", ".join(f'[{v}]' if z else str(v) for i, v, z in dice_sort) + f'] \n= {val}'
        elif isinstance(dice_expr, CoCBPRollExpr):
            if dices is None:
                return f'{dice_expr} = {val}'
            return f'{dice_expr}: [' + ", ".join(f'{{{d[0]},{d[1]}}}' for d in dices) + f'] \n= {val}'
    
    try:
        e = parse(Lexer(expr).tokens)
    except DiceOverflow:
        return ["骰子...太多了，这个我也算不出来", -1]
    except IllegalExpr or TypeError or ValueError:
        return ["掷骰表达式好像有问题，反正我没看明白是啥意思", -1]
    
    ret, summary = eval_expr(e)
    for s in summary:
        return [summarizer(*s), ret]
