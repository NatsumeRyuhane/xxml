...
class Image(Element):
    ...
    def __str__(self):
        if self.base64:
            return f'[Image:{self.base64.decode("utf-8")}]'
        elif self.url:
...
class BotMessage:
    ...
    def __repr__(self):
        return f"{self.msg_type} #{self.id} - {self.target} <- {self.text.__repr__()}"
...
class GroupMessage(Message):
    ...
    def __str__(self):
        return f'#{self.id} {self.group_name}({self.group}) - {self.sender_name}({self.sender}): {self.text.__repr__()}'
