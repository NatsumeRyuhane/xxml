...
class Dice:
    ...
    def __init__(self, count: int, face: int, bonus=None, penalty=None, name="", expr=""):
        ...
        if self.count == 1:
            # bonus/penalty dice cannot be applied simultaneously
            if bonus and penalty:
                raise NotImplementedError

        elif bonus or penalty:
            # bonus/penalty dices cannot be applied to multi-dices
            raise NotImplementedError
...
class DiceExpression:
    ...
    def __str__(self):
        ...
        if len(self.dice_list) == 1 and re.fullmatch("([1-9]\d*)?[dD]([1-9]\d*)+((([bB]([1-9]\d*)?))|(([pP]([1-9]\d*)?)))?", self.raw_expr):
